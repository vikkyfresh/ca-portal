...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        if (!empty($_FILES['photo']['name'])) {
            ...
            if ($_FILES['photo']['error'] !== UPLOAD_ERR_OK) {
                ...
                if ($_FILES['photo']['size'] <= 2 * 1024 * 1024) {
                    $newFile = $uploadDir . 'admin_' . $adminId . '.' . $ext;
                    if (move_uploaded_file($_FILES['photo']['tmp_name'], $newFile)) {
                        $photoPath = 'uploads/passports/admin_' . $adminId . '.' . $ext;
        ...
        if (!$msg) {
            if ($fullName && $email) {
                $pdo->prepare("UPDATE admins SET full_name=?,email=?,phone=?,photo=? WHERE id=?")
                    ->execute([$fullName,$email,$phone,$photoPath,$adminId]);
                ...
                $msg = ['type'=>'error','text'=>'Please enter a valid name and email.'];
            }
        }
        ...
        if (!password_verify($curr, $admin['password_hash'])) {
...
$photoSrc = 'https://ui-avatars.com/api/?name='.urlencode($admin['full_name']).'&background=1e3a8a&color=fff&size=200&bold=true';
if (!empty($admin['photo'])) {
    $sp = dirname(__DIR__).'/'.ltrim($admin['photo'],'/');
    if (file_exists($sp)) $photoSrc = '../'.ltrim($admin['photo'],'/');
}
?>
<!DOCTYPE html>
<html lang="en">
...
.avatar-badge{position:absolute;bottom:2px;right:2px;width:22px;height:22px;background:#f59e0b;border-radius:50%;border:2px solid white;display:flex;align-items:center;justify-content:center}
...
.perm-item{display:flex;align-items:center;gap:8px;padding:9px 12px;background:#f8fafc;border-radius:9px;font-size:13px;color:#334155;border:1px solid #f1f5f9}
.perm-item i{color:#10b981;font-size:.85rem}
...
@media(max-width:900px){.hero-stats{display:none}.info-grid,.form-grid,.perms-grid{grid-template-columns:1fr}}
...
<div class="profile-hero">
    <div class="avatar-wrap" onclick="document.getElementById('photoInput').click()" title="Click to change photo">
        ...
        <div class="avatar-badge"><i class="fas fa-star"></i></div>
    </div>
    <div class="hero-info">
        <div class="hero-name"><?= htmlspecialchars($admin['full_name']) ?></div>
        <div class="hero-role">System Administrator · Dept. of Computer Science · PAAU, Anyigba</div>
        <div class="hero-tags">
    ...
    <div class="hero-stats">
        <div class="hero-stat"><div class="hero-stat-val"><?= $totalStudents ?></div><div class="hero-stat-lbl">Students</div></div>
        <div class="hero-stat"><div class="hero-stat-val"><?= $totalTests ?></div><div class="hero-stat-lbl">Tests</div></div>
        <div class="hero-stat"><div class="hero-stat-val"><?= $totalLecturers ?></div><div class="hero-stat-lbl">Lecturers</div></div>
    </div>
</div>

<!-- Identity -->
<div class="card">
    <div class="card-header"><i class="fas fa-fingerprint"></i><h3>Identity Information</h3><span class="sub">System-assigned</span></div>
    <div class="info-grid">
        <div class="info-item"><div class="i-label">Username</div><div class="i-value"><?= htmlspecialchars($admin['username'] ?? 'admin') ?></div></div>
        <div class="info-item"><div class="i-label">Role</div><div class="i-value">System Administrator</div></div>
        <div class="info-item"><div class="i-label">Department</div><div class="i-value">Computer Science</div></div>
...
<!-- Edit Profile -->
<div class="card">
    <div class="card-header"><i class="fas fa-pen-to-square"></i><h3>Edit Profile</h3><span class="sub">Update your info and photo</span></div>
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="tel" name="phone" value="<?= htmlspecialchars($admin['phone'] ?? '') ?>" placeholder="e.g. 08012345678">
            </div>
        </div>
        <div class="btn-row">
...
<!-- Permissions -->
<div class="card">
    <div class="card-header"><i class="fas fa-shield-halved"></i><h3>Administrator Permissions</h3><span class="sub">Full system access</span></div>
    <div class="perms-grid">
        <div class="perm-item"><i class="fas fa-check-circle"></i> Manage Students &amp; Enrolment</div>
        <div class="perm-item"><i class="fas fa-check-circle"></i> Manage Lecturers</div>
        <div class="perm-item"><i class="fas fa-check-circle"></i> View All Test Results</div>
        <div class="perm-item"><i class="fas fa-check-circle"></i> Export Reports (Excel / PDF)</div>
        <div class="perm-item"><i class="fas fa-check-circle"></i> System Settings &amp; Configuration</div>
        <div class="perm-item"><i class="fas fa-check-circle"></i> Face ID Enrolment Management</div>
        <div class="perm-item"><i class="fas fa-check-circle"></i> Portal Control (Open / Close)</div>
        <div class="perm-item"><i class="fas fa-check-circle"></i> Analytics &amp; Audit Logs</div>
    </div>
</div>
